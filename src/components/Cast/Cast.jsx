import { useEffect, useState } from 'react';
import { useParams } from 'react-router-dom';
import { getFilmCast } from '../../services/api';
import { CastImg, CastList, CastListHeader, CastListItem } from './Cast.styled';

const Cast = () => {
  const [cast, setCast] = useState(null);
  const [error, setError] = useState(null);
  const { movieId } = useParams();

  useEffect(() => {
    async function fetchFilmCast(movieId) {
      try {
        const filmCast = await getFilmCast(movieId);
        setCast(filmCast);
      } catch (error) {
        setError(error);
      }
    }
    fetchFilmCast(movieId);
  }, [movieId]);

  return (
    <>
      <CastList>
        {cast?.cast?.length ? (
          cast.cast.map(actor => (
            <CastListItem key={actor.id}>
              <div>
                <CastImg
                  src={
                    actor.profile_path
                      ? 'https://image.tmdb.org/t/p/w500/' + actor.profile_path
                      : 'data:image/png;base64,'
                  }
                  alt={actor.name}
                />
              </div>
              <CastListHeader>{actor.name}</CastListHeader>
            </CastListItem>
          ))
        ) : (
          <CastListHeader>No cast</CastListHeader>
        )}
      </CastList>
    </>
  );
};

export default Cast;
